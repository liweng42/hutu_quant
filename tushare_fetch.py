...
class TushareFetch(Hutu):
    ...
    def run_only_once(self):
        ...
        filename = os.path.join(const.origin_data_index_day_path, const.CODE_INDEX_SH + '.csv')
    ...
    def get_stock_trade_cal(self, end_date=None):
        ...
        df = self.pro.trade_cal(
            exchange='',
            start_date=const.START_DATE_SH,
            end_date=end_date
            )
        # print(df)
        # 降序排列
        ...
        # 写入交易日历文件
        df.to_csv(const.ORIGIN_DATA_STOCK_TRADE_CAL, header=True, index=False, columns=['exchange', 'cal_date', 'is_open'])
    ...
    def only_once_stock_index_day(self, end_date=None):
        ...
        for index in const.CODE_INDEX_LIST:
            ...
            # print(df)
            filename = os.path.join(const.origin_data_index_day_path, index + '.csv')
            df.to_csv(filename, header=True, index=False, columns=const.COLUMNS)
    ...
    def only_once_all_stock_data(self, end_date):
        ...
        for index, row in stock_list.iterrows():
            ...
            print('code: %s，第%s个/%s秒，共%s个，接口限制：%s 秒内 %s 个' % (row["ts_code"], j, c.seconds, count, max_sec, max_count), end='\r')            
            count = count + 1
        print('\n结束时间：%s' % datetime.now(), end='\n')
    ...
    def daily_job_index_daily_by_date(self, trade_date=None):
        ...
        for li in const.CODE_INDEX_LIST:
            ...
            # print(df)
            if (len(df) > 0):
                filename = os.path.join(const.origin_data_index_day_path, li + '.csv')
                # 存在文件追加数据行
    ...
    def daily_job_stock_daily_by_date(self, trade_date=None):
        ...
        if (len(df) > 0):
            for index, row in df.iterrows():
                ...
                print('进度 : %s [%d/%d]，code:%s' % ((str(percent)+'%', index, len(df), row['ts_code'])), end='\r')
        else:
            print('接口数据为空！', end='\n')
        print('\n结束时间：%s' % datetime.now(), end='\n')
    ...
    def test(self):
        df = self.update_index_daily_by_date()
        # df = self.pro.index_daily(ts_code=const.CODE_INDEX_SH, trade_date='20190312')
        # df = df[1:3]
        print(df)
