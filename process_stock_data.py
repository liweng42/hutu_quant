...
class ProcessStockData(Hutu):
    ...
    def compute_indicators(self, stock_data):
        """
    ...
    def show_compute_index_indicators(self, o_filename, p_filename, code, count, length):
        """
        显示处理进度
        """
        if os.path.exists(o_filename):
            stock_data = pd.read_csv(o_filename)
            stock_data = self.compute_indicators(stock_data)
