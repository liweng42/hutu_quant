...
class EmotionIndex(Hutu):
    ...
    def check_folder(self):
        ...
        if not os.path.exists(const.emotion_index_data_root_path):
            ...
            logger.info('mkdir %s' % const.emotion_index_data_root_path)
        logger.info('emotion_index 目录检测完毕！')
    ...
    def run_only_once(self):
        ...
        """
        # 读取指数文件，取相应的值
        logger.info('=====EmotionIndex run_only_once start=====')
        ...
        logger.info('=====EmotionIndex run_only_once done!=====')
    ...
    def run_daily_job(self):
        ...
        self.compute_emotion_index()
        logger.info('结束时间：%s' % datetime.now())
        logger.info('=====EmotionIndex run_daily_job done!=====')
    ...
    def repeat_daily_job(self, trade_date):
        ...
        """
        # 读取指数文件，取相应的值
        logger.info('=====EmotionIndex repeat_daily_job start=====')
        logger.info('开始时间：%s' % datetime.now())
        ...
        self.compute_emotion_index()
        logger.info('结束时间：%s' % datetime.now())
        logger.info('=====EmotionIndex repeat_daily_job done!=====')
    ...
    def update_emotion_basic(self, trade_date):
        ...
        """
        # 读取指数文件，取相应的值
        # logger.info('=====计算某日市场情绪指标的基础值=====')
        # logger.info('开始时间：%s' % datetime.now())
        # 读取emotion_basic文件
        filename = os.path.join(const.emotion_index_data_root_path,
                                'emotion_basic.csv')
        emotion_df = pd.read_csv(filename)
        # logger.info('计算日期 %s' % trade_date)
        # 将数据按照交易日期升序
        ...
        if (len(tmp_df) <= 0):  # 不存在则追加
            ...
            emotion_df.to_csv(
                filename, index=False, columns=const.EMOTION_BASIC_COLUMNS)
        # logger.info('结束时间：%s' % datetime.now())
        # logger.info('=====计算某日市场情绪指标的基础值 done!=====')
    ...
    #         # logger.info('=====df30_new=====')
    #         # logger.info(df30_new.tail())
    ...
    #         # logger.info('=====df250_new=====')
    #         # logger.info(df250_new.tail())
    #         if (len(df30_new) > 0 and len(df250_new) > 0):
    #             result = max(date_high, df30_new['high'].values[0]) >= df250_new['high'].values[0]
    #             logger.info('%s %s %s %s %s' % (date, date_high, df30_new['high'].values[0], df250_new['high'].values[0], result))
